...
export default function SignupPagePartner() {
  ...
  const dedupeServices = (list: Service[]) => {
    ...
    const fetchServices = async () => {
        try {
            const resData = await api.get(`/services`);

            if (Array.isArray(resData)) {
                const parsed: Service[] = resData
                    .map((s: any) => {
                ...
                console.warn("Unexpected services response shape:", resData);
            }
        } catch (err) {
  ...
  const handleSendOtp = async () => {
    ...
    try {
      ...
      await api.post("/sendMobileOtp", { mobile_number: phone });
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      await api.post("/verifyMobileOtp", { mobile_number: phone, otp });
  ...
  const handleCreateService = async () => {
    ...
    try {
      ...
      if (resData?.id) {
          await fetchServices();
        setSelectedServiceIds((s) => [...s, resData.id]);
        setNewServiceName("");
        setMessage(`✅ Service "${name}" created and selected.`);
      }
    } catch (err: any) {
      setMessage(`❌ ${err.message}`);
    } finally {
  ...
  const handleFinalSubmit = async () => {
    ...
    setMessage("");
    try {
      console.log("[DEBUG Submit] Professional State on Submit:", professional);
      console.log("[DEBUG Submit] Latitude:", professional.clinic_latitude);
      console.log("[DEBUG Submit] Longitude:", professional.clinic_longitude);
      ...
      const data = await api.formDatapost("/registerVet", formData);
